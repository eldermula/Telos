...
/**
 * Synthetics paper open/close — mocked ticks, real DB writers.
 *
 *   node backend/scripts/smoke-synthetic-runtime-paper-offline.js
 */
...
async function main() {
  ...
  const email = `synthetic_offline_${stamp}@telos.test`;
  const userId = (
    await client.query(
  ...
  await client.query(
    `INSERT INTO broker_connections
     ...
     SELECT $1, broker_name, broker_id || '-syn-' || $2::text, encrypted_credentials,
  ...
  assert(
    ...
    `open failed: ${JSON.stringify(openResult && openResult.entryResult)}`
  );
  assert(openResult.trade.asset_class === 'synthetic');
  assert(openResult.trade.symbol === SYMBOL);
  ...
  console.log('SYNTHETIC_RUNTIME_PAPER_OFFLINE_PASS', {
...
main().catch((err) => {
  console.error('SYNTHETIC_RUNTIME_PAPER_OFFLINE_FAIL', err.message || err);
  process.exitCode = 1;
});
