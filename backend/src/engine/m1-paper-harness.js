...
const { evaluateM1Tick, evaluateM1Monitor } = require('./m1-paper-strategy');
...
const DEFAULT_TICK_MS = Number(process.env.M1_PAPER_TICK_MS) || 15000;
const M1_BAR_COUNT = 100;
const MAX_HISTORY = 200;

/**
 * M1 PAPER-ONLY EXPERIMENT harness (docs/15_M1_Forex_Paper_Experiment.md).
 ...
 * M5 session. Restarting the backend clears its history; that is
 * intentional for an explicitly experimental, not-yet-proven tool.
 *
 ...
 * `REAL_TRADING_ENABLED`, confirm-live, M5 real-dispatch, or any admin
 * real-dispatch service. There is no code path here that calls the
 * connector's `/order/*` endpoints — this cannot place a real order
 * even in principle, not just "won't because a flag says paper."
 *
 * Mirrors `m5-paper-harness.js` exactly; the only intentional difference
 * is `timeframe: 'M1'` on getRates.
 */
function createM1PaperHarness(deps = {}) {
  ...
  async function monitorOpenTrade() {
    ...
    const result = evaluateM1Monitor(openTrade, symbolInfo);
    if (!result) return;
  ...
  async function tryOpen() {
    ...
    const result = evaluateM1Tick({ instruments, strategies, balance });
  ...
  function start() {
    ...
    if (typeof timer.unref === 'function') {
      // Prevent a forgotten stop() from keeping the Node process alive
      // during tests (mirrors m5-real-harness.js). Safe in production:
      // the parent process is long-lived either way.
      timer.unref();
    }
    return getStatus();
  }
...
const singleton = createM1PaperHarness();

module.exports = {
  createM1PaperHarness,
