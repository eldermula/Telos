...
/**
 * Crypto Increment E — paper-mode dispatcher for BTC/ETH.
 *
 * Own tick loop. Never imports bot-runtime.js. Never calls MT5 order
 * placement or broker close endpoints. No live-trading env flags /
 * confirm-phrase paths — real crypto orders are a later increment
 * (same sequencing as forex paper → Option 2).
 *
 * Shared writers: trades / decision-log / WS with asset_class='crypto'.
...
const cryptoStrategySelectionService = require('./crypto-strategy-selection.service');
...
const DEFAULT_TICK_MS = Number(process.env.CRYPTO_PAPER_TICK_MS) || Number(process.env.PAPER_TICK_MS) || 2000;
...
class CryptoBotRuntime {
  ...
  constructor(instance, options = {}) {
    ...
    this.strategySelection = options.strategySelection || cryptoStrategySelectionService;
  ...
  async initialize() {
    ...
    const openTrades = await tradesRepository.listOpenCryptoTradesForResume(this.botInstanceId);
  ...
  start() {
    ...
    this.timer = setInterval(() => {
      this.tickOnce().catch((err) => {
        console.error('[crypto-bot-runtime] tick failed:', err.message);
      });
    }, this.tickMs);
  ...
  async _maybeOpenPositionPaper() {
    ...
    const selection = await this.strategySelection.selectCryptoTradeAcrossWatchlist();
    if (!selection) {
      return null;
    }
    ...
    try {
      ...
      console.error('[crypto-bot-runtime] price fetch failed, will retry next tick:', err.message);
      return null;
    }
    if (symbolInfo.bid == null || symbolInfo.ask == null) {
      console.error('[crypto-bot-runtime] MT5 returned no live tick for', symbol);
      return null;
    }

    const spec = normalizeCryptoContractSpec({ ...symbolInfo, symbol });
    if (!spec.sizingReady) {
      await decisionLogRepository.insertDecision({
        botInstanceId: this.botInstanceId,
        decisionType: 'trade_rejected',
        triggeringCondition: 'crypto_contract_spec_not_sizing_ready',
    ...
    // Paper display lot only — P&L is riskedAmount × R-multiple (forex paper model).
    ...
    try {
      ...
      if (err && err.code === '23505') {
        console.warn('[crypto-bot-runtime] open blocked by one_open_trade_per_user');
        return null;
      }
      throw err;
    }
...
/** @type {Map<string, CryptoBotRuntime>} */
const cryptoRuntimes = new Map();

async function startCryptoRuntime(instance, options = {}) {
  const existing = cryptoRuntimes.get(instance.id);
  if (existing) {
    if (!existing.running) {
  ...
  const runtime = new CryptoBotRuntime(instance, options);
  await runtime.initialize();
  cryptoRuntimes.set(instance.id, runtime);
  runtime.start();
  return runtime;
}

async function stopCryptoRuntime(botInstanceId) {
  const runtime = cryptoRuntimes.get(botInstanceId);
  if (!runtime) return;
  await runtime.stop();
  cryptoRuntimes.delete(botInstanceId);
}

function getCryptoRuntime(botInstanceId) {
  return cryptoRuntimes.get(botInstanceId) || null;
}
