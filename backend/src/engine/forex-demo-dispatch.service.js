...
/**
 * Admin-gated, auto-expiring forex demo bypasses:
...
const CACHE_KEY = 'forex:demo-dispatch-config';
const EXPIRY_LOG_KEY_DISPATCH = 'forex:demo-dispatch:expiry-logged';
const EXPIRY_LOG_KEY_CONFIRM = 'forex:demo-confirm:expiry-logged';
const EXPIRY_LOG_KEY_MANUAL = 'forex:demo-manual-trade:expiry-logged';
...
async function fetchFromDatabase() {
  const result = await pool.query(
    `SELECT ${RETURNING_COLS}
     FROM forex_demo_dispatch_config
     WHERE id = 1`
  );
  if (!result.rows[0]) {
    throw new Error('forex_demo_dispatch_config singleton row missing');
  }
...
async function getCachedOrDbRow() {
  ...
  await redis
    ...
    .catch((err) => {
      console.error(`[forex-demo-bypass] cache write failed: ${err.message}`);
    });
  return payload;
}
...
async function maybeLogExpiry(layer, enabledUntilIso, logKey, label) {
  ...
  try {
    ...
    if (set === 'OK') {
      console.warn(
        `[forex-demo-${layer}] EXPIRED — ${label} is now disabled ` +
...
async function invalidateCache() {
  await redis.del(CACHE_KEY).catch((err) => {
    console.error(`[forex-demo-bypass] cache invalidation failed: ${err.message}`);
  });
...
async function isDemoDispatchEnabled() {
  try {
    ...
    console.error(
      `[forex-demo-dispatch] read failed, treating as disabled: ${err.message}`
    );
    return false;
  }
}
...
async function isDemoConfirmEnabled() {
  try {
    ...
    console.error(
      `[forex-demo-confirm] read failed, treating as disabled: ${err.message}`
    );
    return false;
  }
}
...
async function isManualTestTradeEnabled() {
  try {
    ...
    console.error(
      `[forex-demo-manual-trade] read failed, treating as disabled: ${err.message}`
    );
    return false;
  }
}
...
async function enableDispatch(adminUserId, minutes) {
  ...
  const result = await pool.query(
    `UPDATE forex_demo_dispatch_config
  ...
  console.warn(
    `[forex-demo-dispatch] ENABLED by admin_user_id=${adminUserId} ` +
...
async function disableDispatch(adminUserId) {
  ...
  await invalidateCache();
  console.warn(
    `[forex-demo-dispatch] DISABLED early by admin_user_id=${adminUserId}`
  );
...
async function enableConfirm(adminUserId, minutes) {
  ...
  console.warn(
    `[forex-demo-confirm] ENABLED by admin_user_id=${adminUserId} ` +
      `for ${mins} minute(s) until=${status.enabled_until} — ` +
...
async function disableConfirm(adminUserId) {
  ...
  await invalidateCache();
  console.warn(
    `[forex-demo-confirm] DISABLED early by admin_user_id=${adminUserId}`
  );
...
async function enableManualTestTrade(adminUserId, minutes) {
  ...
  console.warn(
    `[forex-demo-manual-trade] ENABLED by admin_user_id=${adminUserId} ` +
      `for ${mins} minute(s) until=${status.enabled_until} — ` +
...
async function disableManualTestTrade(adminUserId) {
  ...
  await invalidateCache();
  console.warn(
    `[forex-demo-manual-trade] DISABLED early by admin_user_id=${adminUserId}`
  );
