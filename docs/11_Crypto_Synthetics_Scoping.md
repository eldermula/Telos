...
**Status: scoped and decided.** Instrument scope: **crypto, BTC/ETH only**, confirmed. Synthetics: explicitly deferred as a distinct future decision, not folded into this scope. Architecture (§0.2) and instrument scope (§6) are both settled. No code written yet — this remains a design document. Implementation begins with a separately scoped first increment, per §6's trigger conditions.
...
|---|---|---|---|
| **2 — Market Intelligence** | ADX/ATR math itself (`bot/market-intelligence`) is generic — works on any OHLC series. But the volatility category thresholds (`<0.8` LOW / `0.8–1.3` NORMAL / `>1.3` HIGH, Section 9's ratio-to-rolling-average design) were calibrated by watching real forex/gold behavior. | Two effects, not one: crypto's baseline vol runs at a different order of magnitude than FX majors — but Section 9's design is a ratio-to-rolling-average, which is self-normalizing to that baseline, so absolute level alone shouldn't break it. The real driver is crypto's sharper, faster regime-switching: volatility clusters and breaks harder than FX, so the ratio crosses the 0.8/1.3 breakpoints more often even after normalizing. Under current thresholds, that likely reads "HIGH" far more than intended, firing the micro circuit breaker's volatility arm more than it should. **Needs its own threshold calibration — worth confirming the ratio-based approach itself still behaves as intended before assuming a simple cutoff shift is enough.** | Synthetic indices have *designed*, statistically fixed volatility — likely needs its own threshold set entirely, calibrated against the instrument's documented profile rather than observed like forex. |
...
3. **Which specific synthetic indices, if synthetics is pursued later?** **Volatility Indices, conditionally** — **Live-verified on Deriv-Demo (login `6255429`, 2026-08-08):** exact MT5 names are spaced strings such as `Volatility 10 Index`, `Volatility 25 Index`, `Volatility 50 Index`, `Volatility 75 Index`, `Volatility 100 Index` (also 5/15/30/90 and `(1s)` variants). `R_10`-style aliases do **not** resolve. Full probe + first-cut thresholds/strategy-fit: `docs/13_Synthetics_Probe_Report.md` (replaces the prior unverified guess). Volatility Indices fit the "continuous designed-vol" premise in §1, but the probe showed ATR-**ratio** regimes are nearly stationary (forex `0.8/1.3` never tags HIGH) while absolute ATR% scales with the index number — so "same logic, new thresholds" needs a regime-model rethink, not only cutoff shifts. Boom/Crash and Jump indices remain a different shape — long calm drift punctuated by designed spikes — and would likely be misread by the existing breakout strategy as genuine breakouts. That's not a threshold-recalibration problem, it's a strategy-logic mismatch: a bigger lift than this document's estimates account for, and out of first-cut scope.
