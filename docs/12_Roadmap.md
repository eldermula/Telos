...
- `BotRuntime` real-mode lifecycle (open + monitor + close reconciliation, the actual fork) — **E**, highest scrutiny of all five, sub-incremented E.0–E.8. ~~E.0–E.4~~ → done (boot tripwire, E1 resolver bypass + Layer 0 invariant, persistence, shared helpers, dispatcher + paper rename; real methods stubbed). **E.5 / E.6 / E.7** each require individual review before the next. E.8 (live demo round-trip) waits on market reopen.
