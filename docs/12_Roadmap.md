...
- ~~Live-trading confirmation endpoint (Layer 2 opt-in, re-required after every Stop)~~ → **D, done.** `POST /trading/session/confirm-live`, 15-minute TTL, Stop-side clear (including no-op Stop), session-shape additions, Confirm Live modal on the Trading page. Smoke: `OPTION2_D_CONFIRM_LIVE_PASS`.
