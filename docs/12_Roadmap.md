...
- Connector hardening (Layer 0 account/type verification) + new capabilities (real equity read, close-time deal history lookup) — **B, code complete, rejection path verified (including the mechanical no-downstream-call proof); live success-path round-trip pending market reopen (weekend closure) — tracked to close out independently, not blocking C.**
- ~~`executionMode` resolver (env kill switch + live account-type read + confirmation timestamp, fails closed)~~ → **C, done.** `backend/src/engine/execution-mode.js`, pure function, no dependency on B's connector work. `REAL_TRADING_ENABLED` env kill switch added (strict exact-string `'true'` parsing). Updated in D to route the confirmation check through `isConfirmationActive` (so D's 15-minute TTL actually gates real-mode resolution). Two Increment E dependencies flagged, not yet built: an accessor for `account_type`/`live_trading_confirmed_at`, and the per-tick (not cached-at-Start) freshness requirement.
- ~~Live-trading confirmation endpoint (Layer 2 opt-in, re-required after every Stop)~~ → **D, done.** `POST /trading/session/confirm-live`, 15-minute TTL, Stop-side clear (including no-op Stop), session-shape additions, Confirm Live modal on the Trading page. Smoke: `OPTION2_D_CONFIRM_LIVE_PASS`. D follow-up: `GET /trading/account-info` proxy so the modal shows live MT5 equity, not the paper ledger (`OPTION2_D_ACCOUNT_INFO_PASS`).
- `BotRuntime` real-mode lifecycle (open + monitor + close reconciliation, the actual fork) — **E**, highest scrutiny of all five, sub-incremented E.0–E.8. ~~E.0–E.8 code~~ → done; **E.8 live PASS pending** market reopen (`10018 Market closed` — weekend). AutoTrading `10027` was cleared after enabling Algo Trading on the terminal; Layer 0/`demo` place path already proven up to `order_send`. Re-run in Monday liquid hours: `node backend/scripts/smoke-option2-e8-live-demo-roundtrip.js`.
