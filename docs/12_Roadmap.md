...
- `BotRuntime` real-mode lifecycle (open + monitor + close reconciliation, the actual fork) — **E**, highest scrutiny of all five, sub-incremented E.0–E.8. ~~E.0~~ → done (production boot refusal if `REAL_TRADING_ALLOW_DEMO` is set at all; strict bypass boolean for non-production E1). E.1–E.7 in progress; E.8 (live demo round-trip) waits on market reopen.
