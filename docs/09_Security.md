...
- **`REAL_TRADING_ENABLED`** — Layer 1 kill switch. Exact-string `'true'` only; anything else (including `'True'` / `'1'` / unset) is off. Absent/off means the automatic loop never dispatches to real-mode methods, regardless of account type or confirmation. Unchanged — this is the real on/off switch, not testing infrastructure.
- **Forex demo confirm / dispatch / manual test-trade (admin toggles)** — retired `REAL_TRADING_ALLOW_DEMO` env + production boot tripwire. Parallel singleton `forex_demo_dispatch_config` (migration `023`) with Redis cache, admin-JWT routes under `/admin/forex/demo-*`, 30-minute max, auto-expiry. Layer 2 (`confirm_enabled_until`) gates demo `confirm-live`; Layer 3 (`enabled_until`) gates demo real dispatch; `manual_test_trade_enabled_until` gates `POST /trading/test-dispatch-real` and `test-close-real`. Same pattern as synthetics' `synthetic_demo_dispatch_config`. **Does not rewrite `expectedAccountType`** — Layer 0 stays honest. Verified by `forex-demo-dispatch.service.test.js`, `confirm-live-demo-bypass.test.js`, `admin-demo-toggle-audit.test.js`.
