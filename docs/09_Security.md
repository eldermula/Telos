...
- **`REAL_TRADING_ALLOW_DEMO`** — E1 non-production verification bypass. Exact-string `'true'` enables dispatch of real-mode *methods* against a `demo` account so the full lifecycle can be proven without real capital. **Controls dispatch only** — `expectedAccountType` passed to `placeOrder`/`closeOrder` must always be the true detected account type; Layer 0 stays honest and untouched by this flag. **Production boot tripwire (E.0):** if `NODE_ENV=production` and `REAL_TRADING_ALLOW_DEMO` is present in the environment at all (any value, including `'false'` or empty string), the backend refuses to start — mandatory *absence*, the inverse of `ACCESS_GATE_*`'s mandatory presence. Verified by `env-real-trading-bypass.test.js`.
