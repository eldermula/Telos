...
- Middleware on every `/api/v1/*` route except `POST /access-gate/verify` and `GET /access-gate/status`. `GET /health` stays outside `/api/v1` and is never gated (uptime — §10 / Phase 8.4). WebSocket `/ws` checks the same cookie when the gate is configured — first line inside `wss.on('connection', ...)` in `websocket-server.js`, before the `?token=` session check even runs. Note the exact mechanics: the HTTP `Upgrade` handshake itself already completes (`101 Switching Protocols`, owned internally by the `ws` library before app code runs) — the gate check then closes the now-open socket immediately after, via a WS Close frame (code `4403`). Not literally the same rejection point as REST (which never returns a success response at all), but functionally equivalent: no `bot-events` subscription, no data, socket dead within milliseconds. Verified end-to-end (no cookie, garbage-token cookie, wrong-secret-signed cookie all → `4403`; valid cookie → `connection.ready`) by `smoke-access-gate-ws-85.js`.
...
### 11a. Option 2 real-trading env gates (Increment E)

- **`REAL_TRADING_ENABLED`** — Layer 1 kill switch. Exact-string `'true'` only; anything else (including `'True'` / `'1'` / unset) is off. Absent/off means the automatic loop never dispatches to real-mode methods, regardless of account type or confirmation.
- **`REAL_TRADING_ALLOW_DEMO`** — E1 non-production verification bypass. Exact-string `'true'` enables (1) dispatch of real-mode *methods* against a `demo` account and (2) Layer 2 `confirm-live` for `account_type==='demo'` so E.8 can arm without a real-money account. **Does not rewrite `expectedAccountType`** — `placeOrder`/`closeOrder` must always receive the true detected account type; Layer 0 stays honest. **Production refusal (two layers):** (a) E.0 boot tripwire in `index.js` — if `NODE_ENV=production` and the env var is present at all (any value), refuse to start; (b) `confirmLiveTrading` itself calls the same `assertRealTradingDemoBypassAllowed` predicate before any I/O, and the demo-acceptance arm additionally requires `NODE_ENV !== 'production'`. So a direct `require` of `trading-engine` (no boot path) still cannot arm demo under production+bypass. Verified by `env-real-trading-bypass.test.js` + `confirm-live-demo-bypass.test.js`.
