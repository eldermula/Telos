 ...
 */
function evaluateProfitLock({ activeTradingBalance, peakEquity, initialBalance, currentTier = 0 }) {
  ...
  const currentTierStepSize = getTierRow(currentTier).stepSize;
  ...
  // 7 — blocks beyond 7 don't need distinct tracking since Tier 7's step
  // size/risk parameters are fixed regardless of how far past it the
  // account goes (Section 3's "7+" row).
  const newTier = Math.min(currentTier + completedBlocks, TIER_MATRIX.length - 1);
